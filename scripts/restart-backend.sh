...
# Restart backend on Debian/Ubuntu-like systems.
...
#
# Usage:
#   ./scripts/restart-backend.sh                # defaults
#   PORT=8080 MODE=dev ./scripts/restart-backend.sh
#   VENV_DIR=/opt/projects/venv PORT=5000 MODE=gunicorn WORKERS=2 THREADS=2 ./scripts/restart-backend.sh
#
# Env vars:
#   PORT        (default 5000)
...
kill_by_cmdpatterns() {
  # Match likely backend processes
  local patterns=(
    'backend/app.py'
    'backend/wsgi.py'
...
info "Stopping existing backend on port $PORT"
